...
const char * const BITCOIN_CONF_FILENAME = "iots.conf";
const char * const BITCOIN_PID_FILENAME = "iotsd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "iots" is a composite category enabling all Iots-related debug output
                if(ptrCategory->count(std::string("iots"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\IotsCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\IotsCore
    // Mac: ~/Library/Application Support/IotsCore
    // Unix: ~/.iotscore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "IotsCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/IotsCore";
#else
    // Unix
    return pathRet / ".iotscore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty iots.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override iots.conf
