...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all Iots related functionality
...
/*
    ...
    allow endless transaction that would bloat Iots and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all Iots specific functionality
