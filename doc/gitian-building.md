...
*Setup instructions for a Gitian build of Iots Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Iots
...
- [Building Iots Core](#building-iots-core)
...
Clone the git repositories for Iots Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/iotspay/iots
...
in the Iots Core repository under 'Fetch and create inputs' to install sources which require
...
To build Iots Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Iots Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/iots/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/iotspay/iots
...
For example:
```bash
URL=https://github.com/crowning-/iots.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit iots=${COMMIT} --url iots=${URL} ../iots/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit iots=${COMMIT} --url iots=${URL} ../iots/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit iots=${COMMIT} --url iots=${URL} ../iots/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the iots git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../iots/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/iotspay/iots-detached-sigs.git

BTCPATH=/some/root/path/iots
SIGPATH=/some/root/path/iots-detached-sigs

./bin/gbuild --url iots=${BTCPATH},signature=${SIGPATH} ../iots/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/iots-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/iots-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/iots-osx-build.assert
```
...
[iots/gitian.sigs](https://github.com/iotspay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
