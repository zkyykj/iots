...
// which are part of the Iots repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="iots-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /iots-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "iotspay-iots/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from iotspay-iots/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'iotspay-iots/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from iotspay-iots/develop"
            }
          }

          if (hasCache) {
            sh "cd /iots-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /iots-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /iots-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /iots-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /iots-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /iots-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /iots-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
